<head>
  <?php include '../../refferences.php'; ?>
</head>
  <?php include '../../header.php'; ?>

  <?php $Libelle = 'ThinkPad T550'; /* Nom PC*/
      $Img1 = '../../Images/Win/lenovo/ThinkPad t550-1.jpg';
      $Img2 = '../../Images/Win/lenovo/ThinkPad t550-2.jpg';
      $Img3 = '../../Images/Win/lenovo/ThinkPad t550-3.jpg';
$Fabriquant = 'Lenovo'; /* Nom Fabriquant*/
$Ports = '3x USB, 1x Ethernet, 1x VGA, 1x Mini DisplayPort, 1x SD'; /* Liste des ports*/
$Sortie = 'HDMI, VGA'; /* Liste des sorties vidéos */
$Dimensions = '2.25 x 38.3 x 25.1 cm'; /* Dimensions */
$TailleEcran = '15,6'; /* Taille de l'écran en pouces*/
$Bluetooth = '1'; /*1 si l'appareil possède le Bluetooth, sinon 0*/
$DVD = '0'; /*1 si l'appareil possède un lecteur DVD, sinon 0*/
$Camera = '1'; /*1 si l'appareil possède une webcam intégrée, sinon 0*/
$Micro = '1'; /*1 si l'appareil possède un microphone, sinon 0*/
$Tactile ='0'; /*1 si l'appareil possède un écran tactile, sinon 0*/
$Batterie = '5h30'; /*Autonomie de la batterie*/
$OS = '0'; /* 0 = Windows, 1 = Mac, 2 = Linux*/
$Processeur = 'Intel Core i5-5300'; /*Modèle de CPU*/
$Ram = '8 Go'; /*Quantité de RAM installée*/
$GPU = 'Intel HD Graphics 5500'; /* Carte graphique*/
$Stockage = 'SSD 512 Go'; 
$Resolution = '1920x1080'; /* Résolution longueur x largeur*/
$Grade = 'B'; /* Grade de l'appareil*/
$Prix = '400'; /* En euro*/
include '../../info.php';?>